...
namespace Mews.Fiscalizations.Spain.Model.Response
{
    public static class ResponseResult
    {
        ...
        public static ResponseResult<T> Error<T>(ErrorResult error)
            where T : class
        {
            return new ResponseResult<T>(error);
        }

        public static ResponseResult<T> Error<T>(string code, string message)
            where T : class
        {
    ...
    public class ResponseResult<TResult> : Coproduct2<TResult, ErrorResult>
        where TResult : class
    {
        internal ResponseResult(TResult successResult = null)
            : base(successResult)
        {
        }

        internal ResponseResult(ErrorResult errorResult = null)
            : base(errorResult)
        {
        }
