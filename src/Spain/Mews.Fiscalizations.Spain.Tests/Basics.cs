...
namespace Mews.Fiscalizations.Spain.Tests.IssuedInvoices
{
    public class Basics
    {
        ...
        private async Task<SimplifiedInvoice> SuccessfullyPostInvoice(Client client)
        {
            ...
            var responseErrorMessages = response.SuccessResult.Get().Invoices.Select(i => i.ErrorMessage).Flatten();
            var errorMessage = String.Join(System.Environment.NewLine, responseErrorMessages);
            Assert.AreEqual(response.SuccessResult.Get().Result, RegisterResult.Correct, errorMessage);
        ...
        private async Task AssertNifLookup(INonEmptyEnumerable<NifInfoEntry> entries, NifSearchResult expectedResult)
        {
            ...
            Assert.AreEqual(response.SuccessResult.Get().Results.Count(), entries.Count());
            foreach (var result in response.SuccessResult.Get().Results)
            {
