...
using Mews.Fiscalizations.TicketBai.Dto;
using FuncSharp;
using System.Linq;
...
namespace Mews.Fiscalizations.TicketBai
{
    ...
    {
        private static IDVersionTicketBaiType1 TicketBaiVersion = IDVersionTicketBaiType1.Item12;
        ...
        private static SignatureType Convert(Signature signature)
        {
            return new SignatureType
            {
                Id = signature.Id,
                // KeyInfo = Convert(signature.KeyInfo),
                // SignedInfo = Convert(signature.SignedInfo),
                // Object = Convert(signature.Object),
                // SignatureValue = Convert(signature.Value)
            };
        }
        ...
        private static IDDestinatario Convert(Receiver receiver)
        {
            ...
            return new IDDestinatario
            {
                ...
                Item = receiver.Match(
                    ...
                    foreign => new IDOtro1
                    {
                        CodigoPais = foreign.Country.Map(c => Convert1(c)).ToNullable(),
        ...
        private static HuellaTBAI1 Convert(InvoiceFooter footer)
        {
            ...
            {
                EncadenamientoFacturaAnterior = footer.PreviousInvoiceChain.Map(c => Convert(c)).GetOrNull(),
                Software = Convert(footer.Software),
                NumSerieDispositivo = footer.DeviceSerialNumber.Map(n => n.Value).GetOrNull()
            };
        }

        private static EncadenamientoFacturaAnteriorType Convert(PreviousInvoiceChain previousInvoiceChain)
        {
            return new EncadenamientoFacturaAnteriorType
            {
                SerieFacturaAnterior = previousInvoiceChain.Series.Map(s => s.Value).GetOrNull(),
                NumFacturaAnterior = previousInvoiceChain.Number.Value,
                FechaExpedicionFacturaAnterior = Convert(previousInvoiceChain.IssueDate),
                SignatureValueFirmaFacturaAnterior = previousInvoiceChain.Signature.Value
            };
        }
        ...
        private static EntidadDesarrolladoraType1 Convert(Developer developer)
        {
            return developer.Match(
                ...
                foreign => new EntidadDesarrolladoraType1
                {
                    ...
                    {
                        CodigoPaisSpecified = foreign.Country.NonEmpty,
                        CodigoPais = foreign.Country.Map(c => Convert(c)).ToNullable(),
                        ID = foreign.Id.Value,
        ...
        private static CountryType2 Convert(Country country)
        {
            var result = country.Alpha2Code.ToEnum<CountryType2>();
            return result.Get();
        }

        private static CountryType21 Convert1(Country country)
        {
        ...
        private static Factura Convert(Invoice invoice)
        {
            return new Factura
            {
                ...
                TipoDesglose = Convert(invoice.TaxBreakdown) // Copy pasted from Spain (SII).
            };
        }
        ...
        private static DatosFacturaType Convert(InvoiceData data)
        {
            return new DatosFacturaType
            {
                ...
                DescripcionFactura = data.Descripton.Value,
                ...
                Claves = data.TaxRegulationTypes.Select(t => Convert(t)).ToArray()
            };
        }
        ...
        private static IDClaveType Convert(TaxRegulationType type)
        {
            return new IDClaveType
            {
                ClaveRegimenIvaOpTrascendencia = type.Match(
                    TaxRegulationType.GeneralTaxRegimeActivity, _ => IdOperacionesTrascendenciaTributariaType.Item01,
                    TaxRegulationType.Export, _ => IdOperacionesTrascendenciaTributariaType.Item02,
                    TaxRegulationType.WorksOfArt, _ => IdOperacionesTrascendenciaTributariaType.Item03,
                    TaxRegulationType.InvestmentGold, _ => IdOperacionesTrascendenciaTributariaType.Item04,
                    TaxRegulationType.TravelAgencies, _ => IdOperacionesTrascendenciaTributariaType.Item05,
                    TaxRegulationType.GroupsOfEntities, _ => IdOperacionesTrascendenciaTributariaType.Item06,
                    TaxRegulationType.CashBasis, _ => IdOperacionesTrascendenciaTributariaType.Item07,
                    TaxRegulationType.CanaryIslandsGeneralIndirectTax, _ => IdOperacionesTrascendenciaTributariaType.Item08,
                    TaxRegulationType.TravelAgencyServicesActingAsIntermediaries, _ => IdOperacionesTrascendenciaTributariaType.Item09,
                    TaxRegulationType.Collections, _ => IdOperacionesTrascendenciaTributariaType.Item10,
                    TaxRegulationType.BusinessPremisesLeaseActivities1, _ => IdOperacionesTrascendenciaTributariaType.Item11,
                    TaxRegulationType.BusinessPremisesLeaseActivities2, _ => IdOperacionesTrascendenciaTributariaType.Item12,
                    TaxRegulationType.BusinessPremisesLeaseActivities3, _ => IdOperacionesTrascendenciaTributariaType.Item13,
                    TaxRegulationType.InvoiceWithVATPendingAccrual1, _ => IdOperacionesTrascendenciaTributariaType.Item14,
                    TaxRegulationType.InvoiceWithVATPendingAccrual2, _ => IdOperacionesTrascendenciaTributariaType.Item15,
                    TaxRegulationType.SurchargeOperations, _ => IdOperacionesTrascendenciaTributariaType.Item51,
                    TaxRegulationType.SimplifiedOperations, _ => IdOperacionesTrascendenciaTributariaType.Item52,
                    TaxRegulationType.NotVatEntity, _ => IdOperacionesTrascendenciaTributariaType.Item53
                )
            };
        }
